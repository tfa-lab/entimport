...
This is a fork of <https://github.com/ariga/entimport> and is customized for us.
...
```shell
go run github.com/tfa-lab/entimport/cmd/entimport
```

- For example, importing a MySQL schema with `users` table:

```shell
go run github.com/tfa-lab/entimport/cmd/entimport -dsn "mysql://root:pass@tcp(localhost:3308)/test" -tables "users"
```
...
```shell
go run github.com/tfa-lab/entimport/cmd/entimport -dsn "postgres://postgres:pass@localhost:5432/test?sslmode=disable" 
```

2. Import ent schema from MySQL database

```shell
go run github.com/tfa-lab/entimport/cmd/entimport -dsn "mysql://root:pass@tcp(localhost:3308)/test"
```
...
```shell
go run github.com/tfa-lab/entimport/cmd/entimport -dsn "..." -tables "users,user_friends" 
```

4. Import to another directory:

```shell
go run github.com/tfa-lab/entimport/cmd/entimport -dsn "..." -schema-path "/some/path/here"
```
